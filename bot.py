...
@client.event
async def on_ready():
    await client.change_presence(activity=discord.Game('n!help|version 1.0 now out!'))
...
async def player(ctx, *, arg):
    ...
    if int(play_time) == 0:
        ...
        embed.add_field(name="Time Played", value="{:,.2f}".format(play_time)+" Days", inline=True)
        embed.add_field(name="Mobs Killed", value="{:,}".format(int(mob_kills)), inline=True)
        ...
        embed.add_field(name="**Helmet**", value=helmet, inline=False)
        embed.add_field(name="**Chestplate**", value=chestplate, inline=False)
        embed.add_field(name="**Leggings**", value=leggings, inline=False)
...
@client.command()
async def help(ctx):
    embed=discord.Embed(title="Command Prefix - n!", description="For more help DM Need_Not#1049", color=0x14b33c)
    embed.set_author(name="Need_Not 2.0 help")
    embed.add_field(name="n!player <playername>", value="Returns Player Stats", inline=False)
    embed.add_field(name="n!ores <playername>", value="Returns the amount of ores mined by the player", inline=False)
    embed.add_field(name="n!mob <playername> <mob>", value="displays stats about the player vs mob", inline=False)
    embed.add_field(name="n!block <playername> <block>", value="Shows stats about that player with that block", inline=False)
    embed.add_field(name="n!item <playername> <item>", value="Shows stats about that player with that item", inline=False)
    embed.add_field(name="n!api", value="Shows all the API enpoints", inline=False)
    embed.add_field(name="Note", value="In Minecraft nothing has a space so 'diamond sword' would become 'diamond_sword'", inline=False)
    embed.set_footer(text="Need_Not 2.0 | Soon to overthrow Need_Not")
    await ctx.send(embed=embed)
client.run(token)
